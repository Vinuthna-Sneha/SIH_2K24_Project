...
const registerUser = async (userData, role, res) => {
  try {
    const { email, psbBadge } = userData;
   
    // Check for existing user by email or psbBadge if the role is 'driver'
    const existingUser = await User.findOne(role === 'driver' ? { $or: [{ email }, { psbBadge }] } : { email });
    if (existingUser) {
      return res.status(400).json({ error: `User with this email ${role === 'driver' ? 'or psbBadge ' : ''}already exists` });
    }
...
router.post('/signup/driver', async (req, res) => {
  const { name, email, psbBadge, routeNumber, password } = req.body;

  if (!name || !email || !psbBadge || !routeNumber || !password) {
    return res.status(400).json({ error: 'All fields are required for driver signup' });
  }

  await registerUser({ name, email, psbBadge, routeNumber: parseInt(routeNumber), password }, 'driver', res);
});
